# GUI/CLI Parity Historical Work Notes

These are historical snapshots, including superseded gaps and test results.
Use [work-in-progress.md](work-in-progress.md) for current implementation status
and [contracts.md](contracts.md) for authoritative requirements.
...
Latest dirty-batch validation (supersedes earlier green transfer results for the
new storage implementation): desktop union38792 compiled but failed9/31 tests
(3 Windows-only tests skipped). Eight failures trace to macOS lacking Java
SecureDirectoryStream, which the new secure spool required; GUI agent owns the
native descriptor implementation needed to restore functionality without weakening
parent/rename checks. One failure reproduced missing Android refresh owner binding.
Root fixed DesktopAndroidAdbClient SUBSCRIPTIONS_REFRESH binding; focused29225
passed public id/active/all requests and preserved explicit stale-owner rejection.
Endpoint document-transfer capability negotiation and context/kind tests are added;
the full large-transfer union must pass again after the spool repair. No new batch
version/prepush/commit receipt exists yet.

Android refresh source freeze:44648 passed40 focused tests. Final scheduling
regression64078 reproduced failure to reschedule a valid request whose route
preparation failed;45060 passed11 refresh tests after the fix. Stale owner/revision
requests still schedule nothing. Android agent is building a fresh release APK for
retained-FGS native verification; earlier cleanup APK does not prove this path.
Only task-owned AVDs5582/5590 are in scope. GUI agent now owns desktop transfer
storage hardening, Android agent owns fresh retained-refresh native checks, and
native installer agent continues protected Windows installer implementation.

Root Android logical-document prerequisite: AndroidControlReader.executeDocument
now keeps large requests/results and correlation while legacy execute remains
bounded until provider negotiation is wired.60729 reproduced missing API;96194
passed reader tests. Large70,000-domain owner import then failed47598 at normalized
request serialization; AndroidSettingsControl now uses ControlDocumentCodec there.
Union79406 passed38 tests (Settings13, Reader14, Refresh11), proving one commit,
exact retry result, >10MiB logical documents and sanitized malformed large input.
AndroidControlTransfers/Provider and DesktopAndroidAdbClient still use the old
bounded transfer path: this prerequisite alone does NOT deliver large Android ADB
transfers. Root owns those remaining adapter changes.

Fresh retained-refresh APK24914 built successfully and was frozen at
/tmp/vpn-android-retained-refresh.vGlv8N/app-release.apk, SHA256
ef5f5728dfdfaf06dfede08a72c6e0e24000471c08f552d3f9510a1c05ff6290.
It predates root's Android logical-document changes above. Agent reports native
API35 public CLI HTTPS refresh success using a disposable CA only in task5590's
zygote trust namespace, then found connected-refresh failure: loader expects
vpn-control-management while generated runtime uses active-verify-in. Active A
and pending B were preserved, but connected fetch failed before IO. Android agent
owns the fast extraction regression, fix, fresh APK and repeated native proof.
This remains a known product failure until verified fixed, not a TLS bypass or
permission to modify host/5580 trust.

Verified checkpoint: `60f8db884588047bea6eb1f108aea3d4c2ade07d` is on
`origin/dev`, version2.1.2. Full prepush830 passed; all five required exact-SHA
workflows succeeded (Fast34029939692, Android34029939631, Linux34029939702,
Windows34029939715, macOS34029939822). Advisory integration34029939666 also passed.
The Windows installer now includes the prepared GUI/CLI image. Supplementary native
MSI mechanics evidence at /tmp/vpn-windows-ci-native.XOkCOr/msi-mechanics-receipt.txt
used retained2.1.0 app sources and direct jpackage; it is not current-source proof.
The successful exact-SHA Windows workflow is the current packaging gate.

User explicitly authorized test VPN connections and installer/elevation prompts
only in agent-owned disposable VMs/emulators. Host VPN, personal data and emulator5580
remain untouched. Replacing the prior synthetic test installation on verified task
AVDs5582/5590 is within that scope; never reset another device or personal app data.
Native Android verification uses the exact60f8 CI APK, SHA256
5e102a459f630deb16379947eaf50af21a872ab14bedf6ed23f189b2f3af5205,
at /tmp/vpn-android-60f8-native.stwefG/artifact/app-release.apk.

Next dirty batch intentionally spans three parallel workstreams: android_session
owns typed Android subscription refresh including downloader/storage/GUI/worker
admission; native_cli_checks owns protected desktop installer handoff and subsequent
operation-specific Windows elevation; root owns logical-document serialization and
large-transfer integration across desktop GUI/CLI and Android ADB. GUI agent runs
independent disposable Android native checks. One Gradle invocation per checkout;
root schedules host Gradle. Root batches documentation/version/prepush/commit and
exact-SHA verification. Full parity remains incomplete; the audits in cli.md and
older unfinished inventories below remain requirements, not delivered features.

Root transfer prerequisite: `ControlDocumentCodec` now handles logical request,
result and value documents without a total transport-size cap; bounded
`ControlProtocolCodec` wrappers retain the unchanged1MiB wire limit and the same
schema/type/depth/duplicate-key/error rules. Snapshot document methods also use
logical codecs for nested retained results. Shared settings/routing inspection no
longer incorrectly use frame-capped parsing. Regression51812 first failed for the
missing APIs; domain regression58142 then reproduced the settings parser cap.
Focused97397 passed30 tests (four new document tests, including >10MiB, accumulated
snapshots, malformed large input and routing/settings). No desktop/Android public
transfer adapter is wired by this prerequisite; root must continue through all
internal postcommit/client codec sites, private-spool ancestry and actual transfers.

Desktop normalized-result migration now covers AppService, ConfigurationResultData,
OperationRunner, HeadlessSession and ControllerOwner logical codecs. Regression2870
returned RUNTIME_FAILED for a70,000-domain routing import; after migration6735 passed
all four ConfigurationResultData tests, including exact committed large result,
same-request retry without a second revision and retained operation snapshot.
Public transport/client codecs and chunk adapters remain to be connected.

Latest root transfer progress supersedes that preceding sentence for desktop only:
DesktopControlDocuments now drives bounded upload/seal/reference/read/discard frames
through the authenticated ActivationServer. Transfer bindings include owner,
authenticated principal, kind and request context; response context is carried in a
bounded preface and checked on download. Large responses defer lifecycle acknowledgment
until client hash verification plus matching acknowledgment, after its reply flush.
Transient owners retain transfers. GUI/client/CLI logical codec sites were migrated.
Wire regression47049 RED became49427 GREEN (>10MiB request/response +activation);
CLI large routing result regression71066 RED became20775 GREEN, including existing
and fresh DesktopRemoteControlSession attachment after the70,000-domain commit.
Context-bound rerun88947 passed6 tests. This is component integration, not native
packaged/process proof. Remaining: explicit old-owner capability negotiation,
adversarial context/ack/expiry/retry/disk-failure tests, and no-overwrite streaming
exports rather than fully materialized client writes. Android large transfers remain
unwired. Spool ancestry is being hardened by GUI agent before shipping:88984 reproduced
unsafe writable/symlink parents and path-based cleanup redirected by parent rename.
Root must not claim current spool has hostile-parent safety until fix/native proof.

Android deadlock fix:60421 deterministic RED;71030 passed33 affected tests.
Frozen local release APK3c0e1881e4869a626963937ac77b2cc5728d80463f4ec1d02c9a1bb0ad9c9b54
at /tmp/vpn-android-deadlock-fixed.rN0Mn6/app-release.apk passed nativeAPI35 active-A
import removal: terminalOK/revision4, one start/one stop, subsequent reads responsive,
no remaining service. Evidence result.txt in that directory. This is dirty-source
frozen-build proof, not exact-SHA CI. Owned5590/fixture79521 stopped;5580 untouched.
API29 nc -n/-z regression was fixed (three Python tests green). Actual traffic token
verification remains unproven. Android agent subsequently owns retained-FGS refresh
changes; these require their own native proof. Blanket foreground-Activity gating
was rejected as an unnecessary loss of scheduled background Find Best behavior.

User approved preserving agent_docs/.Rhistory with an exact local Git exclusion;
/agent_docs/.Rhistory is now in .git/info/exclude, file untouched, shared ignore rules
unchanged. Do not stage that personal/local history file.

Critical native follow-up: current60f8 API29/API35 consent denial/grant and ON/OFF
passed in synthetic task environments; API35 pending-B deletion preserved actual A.
However reconnect followed by valid JSON IMPORT removing active A left
AndroidVpnService Destroying for >90s, with import INCOMPATIBLE_PROTOCOL and later
reads timing out. GUI agent captures bounded thread evidence, then owns fastest
deterministic regression and lifecycle fix; coordinate with Android refresh agent.
Do not count import/live lifecycle as verified. Traffic token proof also remains
unproven despite successful direct emulator-to-fixture access. API29 integration
probe's toybox nc uses unsupported -n/-z flags; add fixture regression before repair.

Installer work chooses original-user nonprivileged staging/installation for Windows
per-user packages, plus a fixed OS-native privileged coordinator for protected
admission/inventory only. Copying writable local JVM/jars into a protected directory
does not make them trusted elevated code. Machine-protected installation workers
still require trusted ancestry before staging. Native agent's initial handoff
coordinator regression47242 RED /19174 GREEN is only a prerequisite, not a completed
public INSTALL operation. Android refresh work is concurrently under implementation.
