...
Use a reachable SSH host and the loopback relay from `docs/ssh-routing.md`:

1. Import a dedicated private key, paste the verified host key, enable the route, and connect to a known-good VPN location.
2. Confirm the resulting public address belongs to the selected VPN, not the local network ISP, proving both relay legs are active.
3. Refresh a subscription while connected and confirm it succeeds through the active session.
4. Disconnect without disabling SSH Routing, refresh again, and confirm it succeeds through the SSH relay.
5. Stop the SSH relay and confirm refresh fails without a direct-network fallback.
