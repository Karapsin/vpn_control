...
- `gradle.properties` owns `vpnControlVersion` in `a.b.c.d` form.
- Every component is in `0..19`; increment with carry, so `1.3.6.19` becomes `1.3.7.0`.
...
- Android version code uses the four components as a base-20 number. Desktop package projections are generated by `scripts/version_metadata.py`; do not restore commit-count versions.
