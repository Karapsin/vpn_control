...
- **VISUAL-001 — Supported-platform gate.** A release requires successful Android, Linux, Windows, and macOS visual jobs for the exact release SHA. A missing, offline, cancelled, or failed runner blocks release.
...
- **VISUAL-003 — Canonical hosts.** Android uses Pixel 6/API 35 portrait; desktop uses 1280x800 at 100%. Windows UAC/MSI scenes use the `vpn-control-win11` libvirt guest; Linux covers both tray backends; macOS capture has Screen Recording and Accessibility authorization.
...
- **VISUAL-006 — Evidence.** Every run preserves actual screenshots, baseline/actual/diff contact sheets, per-scene diffs, geometry data, JSON summary, and machine fingerprint as workflow artifacts, including on failure.
- **VISUAL-007 — Baseline updates.** Release verification is read-only. Baselines are recorded only by an explicit development operation after a complete platform scene set is captured; no human approval is part of the default release gate.
- **VISUAL-008 — Fleet identity.** Runners use `[self-hosted,vpn-control-visual,android]`, `[self-hosted,vpn-control-visual,linux]`, `[self-hosted,vpn-control-visual,windows-vm]`, and `[self-hosted,vpn-control-visual,macos]`. Runner registration and first baseline capture are operator-owned external setup.
...
- **RELEASE-005 — Exact-SHA release gates.** Release merge fast-forwards `main` from the already verified `origin/dev` SHA, then dispatches exhaustive `VPN Integration / all` and `Visual Regression` for that SHA. Both and all exact-SHA package workflows must succeed.
- **RELEASE-006 — Publisher defense in depth.** The manual publisher independently verifies main/dev SHA equality, version/changelog metadata, exact-SHA package workflows, exhaustive VPN integration, visual regression, stable Android signer, package contents, checksums, and update manifest before creating a new immutable tag/release.
- **RELEASE-007 — No subjective exception.** Visual evidence can be downloaded for diagnosis, but default release readiness is decided entirely by automated comparison/geometry results. There is no manual approval override.
