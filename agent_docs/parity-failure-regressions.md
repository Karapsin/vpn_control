...
| --- | --- | --- | --- | --- | --- | --- |
| Linux static preflight approves a missing workspace parent | Fixture admission | The fixed runner requires an existing owned real parent, but static admission only checked package bytes and protected owner. | Guest and MCP preflight suites now cover ownedWorkspace identity, missing paths, symlinks, UID and protected overlap; portable control tests isolate POSIX admission and run with getuid absent. Routine hygiene and agent tests include them. | checkpoint173/linux-batch/workspace-preflight-red-green.json records pre-change ready:true for missing root; workspace-focused-green.log records corrected selection. | CP173 runner exited at input validation; no scenario receipt exists, so collector reports unknown rather than invented cleanup. | Fresh correlation uses an already admitted parent; no automatic arbitrary directory creation or replay. |
| Script-mode MCP imports a same-named guest helper | MCP integration | scripts precedes agent_tools in the inherited module search path; both preflight and batch adapters selected the guest module. | test_vm_workflow fresh-process script-mode regression sweeps all13 native adapters and exercises preflight/batch planning; routine agent-tool discovery. | checkpoint173/import-fix/red-script-mode-native-adapter-shadow.txt reproduces AttributeError before mutation; green-script-mode-native-adapter-shadow.txt passes12 focused tests; full agent suite338tests/4skips. | CP173 preflight passed after the first fix; batch-plan exposed the second route before the class-wide correction. | Native scenario acceptance remains separate; no uncertain job is replayed. |
| Pinned actionlint download connection reset aborts CI | CI dependency transport | A transient ECONNRESET had no bounded retry; pinned checksum policy was unaffected. | test_check_workflow_syntax covers reset then success, partial bytes, exhaustion and unchanged old cache on checksum failure; routine hygiene. | checkpoint173/actionlint-retry/causal-red-replay.log reproduces exact pre-change RuntimeError after one reset; green.log passes8 tests. Original missing-constant RED was noncausal and labelled accordingly. | Fast Checks36119085476 for d196525 failed at download, then was explicitly rerun once. | Exact-SHA replacement CI still required; invalid bytes never get retry or publication. |
| Linux fixture preflight reads the wrong workspace level | Fixture | DesktopStateStore persists validation and refresh settings under persisted_state; the probe expected root fields. | test_native_fixture_preflight_script uses the real nested shape, boolean-only diagnostics and executes settings cases with getuid absent; routine release hygiene. | CP172 native preflight rejects settings before ON; quick schema RED precedes fix; checkpoint172/preflight-green.log passes24 focused tests. | checkpoint172/linux-batch/scenario-receipt.json confirms TLS passed and complete scoped cleanup with protected owner preserved. | New immutable bundle/correlation must rerun scheduled refresh; installed2.1.17 is not final-source acceptance. |
| Mac monitor unit mocks assume POSIX getsid on Windows | Test fixture | Mocking a missing os.getsid failed before exercising monitor behavior. | Both mocked monitor tests now use an OS facade without getsid on every host, then explicitly create the fake method; routine release hygiene. | checkpoint169/monitor-windows-red.log reproduces both AttributeErrors; monitor-windows-green.log passes13 tests. | Windows CI36109544592 failed during hygiene, before packaging. | Replacement exact-SHA Windows CI required. |
| Android document response loss drops authenticated owner identity | Product CLI transport | Outer exception rendering discarded the controller bound before submit; operation metadata was available only in unread result bytes. | Native-shaped document-read-loss test in DesktopAndroidDocumentClientTest; provider metadata tests in AndroidControlDocumentsTest, both in broad prepush. | checkpoint168/android-document-owner-red.log: expected owner but null. checkpoint168/android-document-green3.log passes40 client/provider tests after preserving known-operation precedence and separating transport uncertainty from normal provider wait polling. | CP166 API35 on frozen2.1.16 commits once, identical request retry preserves operation/revision, full routing restored; root-review.json verifies full content. | New-source native response-loss rerun remains required. |
| Repeated base/target signing setup recreates one keychain | Fixture build integration | Both immutable stages source the signing helper, which previously always created the same keychain. | `test_setup_macos_signing.py` executes both stages against an owned fake security tool and rejects unmarked existing keychains; routine hygiene includes it, POSIX execution explicitly skipped on Windows. | Pre-fix second create returned42; focused two-test suite passes with marked-keychain reuse and certificate-file cleanup. | Real signed pair is not yet built; normal unsigned policy is preserved. | Optional same-source Mac pair workflow must run after push; metadata recorder verifies authority and identity-safe DMG cleanup. |
| Brief warning pressure stops an otherwise admitted macOS VM | Native resource monitor | One warning-level sample caused immediate shutdown; it did not establish exhausted usable RAM. | Actual monitor regression rejects stop for warning then normal; deterministic warning-duration/reset test and native-process critical-stop tests remain in routine release hygiene. | checkpoint167/mac-pressure-red.log proves the old stop invocation; mac-headroom-green.log runs13 tests successfully. | Initial monitor stopped the exact guest cleanly; revised monitor requires30 continuous warning seconds plus less than2GiB free/inactive/speculative page headroom, with immediate critical/nonrecognized pressure stop. Fresh admission remains normal pressure plus coordinator disk/memory checks. | Grace is conservative protection, not a claim that sustained pressure is harmless; preserve all stop receipts. |
| Adopted SSH recovery socket expires and stale intent blocks renewal | MCP lifecycle | Legacy host-only intent retained the pre-adoption configured socket identity; next recovery rejected it as foreign despite known ready completion. | `test_adopted_ready_master_can_recover_after_expiry_without_replacing_history` and unknown/foreign legacy guards in routine agent-tools tests. | checkpoint166/ssh-adoption-red.log reproduces recovery_unavailable; ssh-adoption-green.log passes after route-keyed immutable history. | Existing master restored using the configured key/passphrase; read-only recovery confirms configured_master_ready. | No socket is deliberately terminated to exercise expiry; preserve unknown outcomes and exact native identities. |
| Windows CONNECT capacity response resets before the client receives503 | Build relay fixture | Closing with unread CONNECT bytes triggered Winsock reset; a relative per-read timeout also allowed slow input to extend draining. | `test_guest_build_connect_proxy.py` covers consume-before-response and an absolute one-second deadline; routine release hygiene. | CP165 causal ordering/drip-feed RED; current19 tests GREEN. | Windows CI36033866043 on b542 exposed reset; replacement Windows CI remains required. | Local tests do not establish native Winsock success. |
| Linux lifecycle observer assumes static configuration path or one mixed inbound | Native fixture | Installed runtime uses candidate directories and separate management/user mixed inbounds. | `test_linux_fixture_runtime_config.py` follows owned process ancestry and selects loopback mixed-in; routine hygiene. | checkpoint162/linux-rpm-lifecycle legacy-filename-causal-red.log and all-mixed-inbounds-causal-red.log; five GREEN. | Installed RPM2.1.17 delivered300/300 frames across close/crash/reattach; root-review.json verifies identities and OFF/quit. | Final-source VPN and scheduled-refresh scenarios remain separate. |
| Hygiene stops after first independent failure | Check orchestration | Shell errexit hid later independent portability failures until another push. | `test_release_hygiene_runner.py` executes the actual runner function and checks aggregation, success and cancellation; routine hygiene. | checkpoint162/hygiene165-red.log uses original pass-through fail-fast behavior; three GREEN. | This improves discovery, not product behavior. | Full prepush and exact-SHA workflows still required. |
| Archive restore loses executable permission bits | Evidence restoration fixture | Private extraction umask changed child modes; content-only comparison was insufficient. | `test_native_fixture_archive.py` rejects mode loss and compares tar headers/restored manifests; routine hygiene. | CP166 old extraction differs; preserved-mode extraction matches all five sources. | Copies stay under0700 private parents and0600 archives; no local deletion yet. | Root must finish verifier review and compare current local sources before cleanup. |
