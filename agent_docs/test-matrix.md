...
| Android SDK/AVD admission | `python3 scripts/test_android_avd_sdk_preflight.py` checks wrong-root symlink rejection, real wrapper root selection, private AVD environment and exact image metadata. Release hygiene runs it; the executable shell-wrapper scenario explicitly skips on Windows. |
