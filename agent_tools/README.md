...
1. Call `prepare_start(task, area)` before normal repository inspection, edits, or tests. It fetches `origin/main`, changes branches or fast-forwards only when the worktree is clean, preserves a synchronized dirty `main`, and rebuilds the local docs index.
...
4. Run `run_checks(level="prepush")` after the final content change. A successful run writes a content fingerprint to `.rag_index/prepush_receipt.json`.
5. Use `git_workflow` to push or to resume checks for a full commit SHA. It queries only runs attached to that exact SHA and requires every workflow in `.github/required-workflows.json` to succeed.

`prepare_start` deliberately blocks when a fetch fails, branches diverge, a dirty non-main branch would need switching, or a dirty behind-main worktree would need pulling. Resolve the reported condition explicitly and rerun it.
...
| `git_workflow(action, message=None, paths=None, sha=None)` | Commit explicit safe paths, push, and/or watch exact-SHA CI. |
