...
| Main state/actions | `app/src/main/java/com/kardinal/vpncontrol/MainViewModel.kt`, focused `Android*ActionsService` classes, `AndroidControllerEffectHandler.kt` plus shared controller | `desktopApp/src/main/kotlin/com/kardinal/vpncontrol/desktop/DesktopAppServiceFactory.kt`, `DesktopAppService.kt`, and focused desktop services including `DesktopSettingsService.kt` | `shared/core/src/commonMain/kotlin/com/kardinal/vpncontrol/MainController.kt`, `MainUiState.kt`, `MainUiStateProjector` |
...
| User-visible status or log text | Model status domain facades such as `ConnectionStatusMessages`, `SubscriptionStatusMessages`, `BenchmarkStatusMessages`, `LocationStatusMessages`, `RoutingStatusMessages`, `DiagnosticsStatusMessages`, `RuntimeStatusMessages`, `SettingsStatusMessages`; compatibility facade `StatusMessages`; `StatusMessageTypes.kt`, `StatusMessageCodec.kt`, `StatusMessageKeySelectors.kt`; UI renderer `StatusMessageRenderer.kt`; `i18n-status/*.json`; status renderer/coverage tests; `scripts/status_catalog_tool.py`; `docs/localization.md`. |
