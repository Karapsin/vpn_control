...
| Profile parsing | Android uses the shared parser from app workflows | Desktop uses the shared parser from service workflows | `ProxyParser.kt` facade, `ProxyLinkParser.kt`, `ProxyLinkEncoder.kt`, `JsonSubscriptionParser.kt`, `ProxyParserEngine.kt` subscription orchestration, `VlessParser.kt` compatibility shim |
...
| --- | --- |
| Subscription payload or direct-link parsing | `ProxyParser`, `ProxyLinkParser`, `ProxyLinkEncoder`, `JsonSubscriptionParser`, `ProxyParserEngine`, `VlessParser` compatibility shim, parser tests, import/export behavior, `docs/sing-box-contract.md`, Android and desktop smoke docs. |
