...
VPN Control can connect to a selected VPN location through an SSH server on a remote network:

```text
device -> pinned SSH connection -> loopback-only SSH relay -> selected VPN -> internet
```
...
- A pinned public host key. Obtain it through a trusted path. For example, run `ssh-keyscan -p 228 example.com` from a network where you can verify the result, then compare its fingerprint with `ssh-keygen -lf` on the SSH host.
...
- Without an active session, downloads use the SSH relay when SSH Routing is enabled; otherwise they connect directly.
- A failed active session may fall back to the SSH route only when that route is enabled. A failed SSH route never silently falls back to the public network.
...
Custom sing-box configs are rewritten only when every outbound path is recognized and can be chained through the SSH route. VPN Control rejects unknown outbound types instead of risking a direct leak.
