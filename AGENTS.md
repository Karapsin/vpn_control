...
- Do not add rendered English sentence templates for typed statuses in Kotlin. Add a `structured` catalog key in English, mirror that key in every other status catalog, and render it through `StatusMessages`. Put non-trivial variant selection in `StatusMessageKeySelectors.kt`. Use `./scripts/status_catalog_tool.py add-structured ...` and `./scripts/status_catalog_tool.py check` for typed status catalog changes.
