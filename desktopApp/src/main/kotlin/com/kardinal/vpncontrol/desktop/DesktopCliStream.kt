...
internal object DesktopCliStream {
    fun run(invocation: ControlCliParseResult.Invocation,
            transport: (ControlRequest, Long) -> DesktopCliResponse,
        ...
        var ownerId: String? = invocation.client.controllerId
        ...
        fun emit(result: ControlResult) {
            ...
            } else progress(ControlDocumentCodec.encodeValues(result.data))
        }
        fun failure(code: ControlCode): Int {
            val result = previous?.copy(requestId = UUID.randomUUID().toString(), code = code, final = true,
                ...
                ?: ControlDocumentCodec.decodeResult(desktopCliJsonFailure(code, UUID.randomUUID().toString()).message)
            emit(result)
        ...
        if (invocation.operation !in setOf(ControlOperationId.STATUS, ControlOperationId.STATS, ControlOperationId.LOGS) ||
            invocation.flags != expectedFlags || invocation.client.copy(json = false, timeoutSeconds = 600,
                android = false, serial = null, stateDirectory = null, controllerId = null) != ControlClientOptions()) {
