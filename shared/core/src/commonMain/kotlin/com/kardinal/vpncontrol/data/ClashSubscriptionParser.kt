...
internal object ClashSubscriptionParser {
    ...
    private fun parseProxyEntries(rawBody: String): List<Map<String, String>> {
    ...
    private fun parseProxy(entry: Map<String, String>): ProxyProfile {
        return when (entry.clashString("type").lowercase()) {
            "vless" -> parseVless(entry)
            "vmess" -> parseVmess(entry)
            "trojan" -> parseTrojan(entry)
            "ss", "shadowsocks" -> parseShadowsocks(entry)
            "socks", "socks5" -> parseSocks(entry)
            else -> error("Unsupported Clash proxy protocol")
        }
    }

    private fun parseVless(entry: Map<String, String>): ProxyProfile {
    ...
    private fun parseVmess(entry: Map<String, String>): ProxyProfile {
    ...
    private fun parseTrojan(entry: Map<String, String>): ProxyProfile {
    ...
    private fun parseShadowsocks(entry: Map<String, String>): ProxyProfile {
    ...
    private fun parseSocks(entry: Map<String, String>): ProxyProfile {
