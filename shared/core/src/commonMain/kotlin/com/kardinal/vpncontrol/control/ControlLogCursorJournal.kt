...
/** Pure owner-scoped cursor history. Platform owners serialize publication and reads. */
class ControlLogCursorJournal(initial: List<ConnectionLogEntry>, private val prefix: String, private val capacity: Int = 200) {
    ...
    init { require(capacity > 0 && prefix.isNotBlank()); sync(initial) }

    fun entries(): List<ConnectionLogEntry> = rows.map { it.entry }

    /** A durable publication could not be observed. Never silently bridge that loss. */
    fun markHistoryGap() { next(); observedEntries = null }

    fun sync(entries: List<ConnectionLogEntry>): List<ConnectionLogEntry> {
    ...
    fun read(arguments: Map<String, ControlValue>): Result<Map<String, ControlValue>> = runCatching {
